...
export default {
  ...
  methods: {
    async handleSubmit(event) {
      ...
      try {
        const isValid = this.isValidFormRg;
        if (!isValid) return;
        // Kiểm tra xem tài khoản đã tồn tại hay chưa
        const isExistingAccount = await this.checkExistingAccount(this.email);
        ...
        console.error(
          "Lỗi khi lưu thông tin tài khoản Google vào Firestore:",
          error
        );
      }
    },
